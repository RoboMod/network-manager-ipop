...
unquote (const char *line, char **leftover)
{
	...
	/* Unquote stuff using ipop unquoting rules */
...
do_import (const char *path, char **lines, GError **error)
{
	...
	for (line = lines; *line; line++) {
		...
		if (!strncmp (*line, DEV_TAG, strlen (DEV_TAG))) {
			...
			if (nitems == 1) {
				if (g_str_has_prefix (items[0], "tun")) {
					...
					nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_TAP_DEV, "yes");
				else
			...
			if (nitems == 1) {
				...
				if (!strcmp (items[0], "udp")) {
					/* ignore; udp is default */
				           ...
				           || !strcmp (items[0], "tcp")) {
					nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_PROTO_TCP, "yes");
				} else
		...
		if (!strncmp (*line, MSSFIX_TAG, strlen (MSSFIX_TAG))) {
			nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_MSSFIX, "yes");
			continue;
		}
		...
		if (!strncmp (*line, TUNMTU_TAG, strlen (TUNMTU_TAG))) {
			...
			if (nitems == 1) {
				...
				if ((errno == 0) && (secs >= 0) && (secs < 0xffff)) {
					...
					nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_TUNNEL_MTU, tmp);
					g_free (tmp);
				} else
		...
		if (!strncmp (*line, FRAGMENT_TAG, strlen (FRAGMENT_TAG))) {
			...
			if (nitems == 1) {
				...
				if ((errno == 0) && (secs >= 0) && (secs < 0xffff)) {
					tmp = g_strdup_printf ("%d", (guint32) secs);
					nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_FRAGMENT_SIZE, tmp);
					g_free (tmp);
				} else
					g_warning ("%s: invalid size in option '%s'", __func__, *line);
			} else
		...
		if (!strncmp (*line, COMP_TAG, strlen (COMP_TAG))) {
			nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_COMP_LZO, "yes");
			continue;
		}
		...
		if (!strncmp (*line, RENEG_SEC_TAG, strlen (RENEG_SEC_TAG))) {
			...
			if (nitems == 1) {
				...
				if ((errno == 0) && (secs >= 0) && (secs <= 604800)) {
					tmp = g_strdup_printf ("%d", (guint32) secs);
					nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_RENEG_SECONDS, tmp);
					g_free (tmp);
				} else
		...
		if (   !strncmp (*line, HTTP_PROXY_RETRY_TAG, strlen (HTTP_PROXY_RETRY_TAG))
			...
			nm_setting_vpn_add_data_item (s_vpn,
			                              g_strdup (NM_IPOP_KEY_PROXY_RETRY),
			                              g_strdup ("yes"));
			continue;
		}
		...
		if ((http_proxy || socks_proxy) && !proxy_set) {
			...
			if (nitems >= 2) {
				...
				if (success && proxy_type) {
					nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_PROXY_TYPE, proxy_type);

					nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_PROXY_SERVER, items[0]);
					nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_PROXY_PORT, s_port);
					if (user)
						nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_HTTP_PROXY_USERNAME, user);
					if (pass) {
						nm_setting_vpn_add_secret (s_vpn, NM_IPOP_KEY_HTTP_PROXY_PASSWORD, pass);
		...
		if (!strncmp (*line, REMOTE_TAG, strlen (REMOTE_TAG))) {
			...
			if (nitems >= 1 && nitems <= 3) {
				nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_REMOTE, items[0]);
				have_remote = TRUE;

				if (nitems >= 2) {
					tmp = parse_port (items[1], *line);
					if (tmp) {
						nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_PORT, tmp);
						g_free (tmp);
		...
		if (   !strncmp (*line, PORT_TAG, strlen (PORT_TAG))
			...
			if (nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_PORT))
				continue;
		...
		if ( handle_path_item (*line, PKCS12_TAG, NM_IPOP_KEY_CA, s_vpn, default_path, NULL) &&
		     handle_path_item (*line, PKCS12_TAG, NM_IPOP_KEY_CERT, s_vpn, default_path, NULL) &&
		     handle_path_item (*line, PKCS12_TAG, NM_IPOP_KEY_KEY, s_vpn, default_path, NULL))
			continue;

		if (handle_path_item (*line, CA_TAG, NM_IPOP_KEY_CA, s_vpn, default_path, NULL))
			continue;

		if (handle_path_item (*line, CERT_TAG, NM_IPOP_KEY_CERT, s_vpn, default_path, NULL))
			continue;

		if (handle_path_item (*line, KEY_TAG, NM_IPOP_KEY_KEY, s_vpn, default_path, NULL))
			continue;

		if (handle_path_item (*line, SECRET_TAG, NM_IPOP_KEY_STATIC_KEY,
		                      s_vpn, default_path, &leftover)) {
			handle_direction ("secret",
			                  NM_IPOP_KEY_STATIC_KEY_DIRECTION,
			                  leftover,
			                  s_vpn);
			g_free (leftover);
			have_sk = TRUE;
		...
		if (handle_path_item (*line, TLS_AUTH_TAG, NM_IPOP_KEY_TA,
		                      s_vpn, default_path, &leftover)) {
		...
		if (!strncmp (*line, CIPHER_TAG, strlen (CIPHER_TAG))) {
			...
			if (nitems == 1)
				nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_CIPHER, items[0]);
			else
				g_warning ("%s: invalid number of arguments in option '%s'", __func__, *line);
		...
		if (!strncmp (*line, TLS_REMOTE_TAG, strlen (TLS_REMOTE_TAG))) {
			...
			if (unquoted) {
				nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_TLS_REMOTE, unquoted);
				g_free (unquoted);
			} else
		...
		if (!strncmp (*line, IFCONFIG_TAG, strlen (IFCONFIG_TAG))) {
			...
			if (nitems == 2) {
				nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_LOCAL_IP, items[0]);
				nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_REMOTE_IP, items[1]);
			} else
				g_warning ("%s: invalid number of arguments in option '%s'", __func__, *line);
		...
		if (!strncmp (*line, AUTH_TAG, strlen (AUTH_TAG))) {
			...
			if (nitems == 1)
				nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_AUTH, items[0]);
			else
				g_warning ("%s: invalid number of arguments in option '%s'", __func__, *line);
			g_strfreev (items);
			continue;
	...
	if (!have_client && !have_sk) {
		g_set_error (error,
		             ...
		             "The file to import wasn't a valid IPOP client configuration.");
		...
		g_set_error (error,
		             IPOP_PLUGIN_UI_ERROR,
		             IPOP_PLUGIN_UI_ERROR_FILE_NOT_IPOP,
		             "The file to import wasn't a valid IPOP configure (no remote).");
		g_object_unref (connection);
		connection = NULL;
	} else {
		gboolean have_certs = FALSE, have_ca = FALSE;

		if (nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_CA))
			have_ca = TRUE;

		if (   have_ca
		    && nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_CERT)
		    && nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_KEY))
			have_certs = TRUE;

		/* Determine connection type */
		if (have_pass) {
			if (have_certs)
				ctype = NM_IPOP_CONTYPE_PASSWORD_TLS;
			else if (have_ca)
				ctype = NM_IPOP_CONTYPE_PASSWORD;
		} else if (have_certs) {
			ctype = NM_IPOP_CONTYPE_TLS;
		} else if (have_sk)
			ctype = NM_IPOP_CONTYPE_STATIC_KEY;

		if (!ctype)
			ctype = NM_IPOP_CONTYPE_TLS;

		nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_CONNECTION_TYPE, ctype);
		...
		if (have_certs) {
			...
			key_path = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_KEY);
...
do_export (const char *path, NMConnection *connection, GError **error)
{
	...
	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_CONNECTION_TYPE);
	if (value && strlen (value))
	...
	if (!strcmp (connection_type, NM_IPOP_CONTYPE_STATIC_KEY)) {
	...
	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_TLS_REMOTE);
	if (value && strlen (value))
		tls_remote = value;

	/* Advanced values start */
	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_PORT);
	if (value && strlen (value))
		port = value;

	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_RENEG_SECONDS);
	if (value && strlen (value)) {
		reneg_exists = TRUE;
		reneg = strtol (value, NULL, 10);
	}

	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_PROTO_TCP);
	if (value && !strcmp (value, "yes"))
		proto_udp = FALSE;

	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_TAP_DEV);
	if (value && !strcmp (value, "yes"))
		device_tun = FALSE;

	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_COMP_LZO);
	if (value && !strcmp (value, "yes"))
		use_lzo = TRUE;

	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_CIPHER);
	if (value && strlen (value))
		cipher = value;
	...
	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_TA);
	if (value && strlen (value))
		tls_auth = value;

	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_TA_DIR);
	if (value && strlen (value))
		tls_auth_dir = value;
	...
	if (   !strcmp(connection_type, NM_IPOP_CONTYPE_PASSWORD)
	    || !strcmp(connection_type, NM_IPOP_CONTYPE_PASSWORD_TLS))
	...
	if (!strcmp (connection_type, NM_IPOP_CONTYPE_STATIC_KEY)) {
		if (static_key) {
			...
			g_warning ("%s: invalid ipop static key configuration (missing static key)", __func__);
	}
	...
	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_MSSFIX);
	if (value && strlen (value)) {
		if (!strcmp (value, "yes"))
			fprintf (f, MSSFIX_TAG "\n");
	}

	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_TUNNEL_MTU);
	if (value && strlen (value))
		fprintf (f, TUNMTU_TAG " %d\n", (int) strtol (value, NULL, 10));

	value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_FRAGMENT_SIZE);
	if (value && strlen (value))
	...
	/* Proxy stuff */
	proxy_type = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_PROXY_TYPE);
	if (proxy_type && strlen (proxy_type)) {
		proxy_server = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_PROXY_SERVER);
		proxy_port = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_PROXY_PORT);
		proxy_retry = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_PROXY_RETRY);
		proxy_username = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_HTTP_PROXY_USERNAME);
		proxy_password = nm_setting_vpn_get_secret (s_vpn, NM_IPOP_KEY_HTTP_PROXY_PASSWORD);
