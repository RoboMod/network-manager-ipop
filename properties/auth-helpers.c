          ...
          gpointer user_data)
{
	...
	if (s_vpn) {
		value = nm_setting_vpn_get_data_item (s_vpn, NM_IPOP_KEY_USERNAME);
		if (value && strlen (value))
	...
	widget = setup_secret_widget (builder, tmp, s_vpn, NM_IPOP_KEY_PASSWORD);
	g_free (tmp);
	gtk_size_group_add_widget (group, widget);
	g_signal_connect (widget, "changed", G_CALLBACK (changed_cb), user_data);

	init_one_pw_combo (builder, s_vpn, prefix, NM_IPOP_KEY_PASSWORD, widget, changed_cb, user_data);
}
...
update_pw (GtkBuilder *builder, const char *prefix, NMSettingVPN *s_vpn)
{
	...
	str = gtk_entry_get_text (GTK_ENTRY (widget));
	if (str && strlen (str))
		nm_setting_vpn_add_data_item (s_vpn, NM_IPOP_KEY_USERNAME, str);
	...
	str = gtk_entry_get_text (GTK_ENTRY (widget));
	if (str && strlen (str))
		nm_setting_vpn_add_secret (s_vpn, NM_IPOP_KEY_PASSWORD, str);
	...
	nm_setting_set_secret_flags (NM_SETTING (s_vpn), NM_IPOP_KEY_PASSWORD, pw_flags, NULL);
}
...
static const char *advanced_keys[] = {
	NM_IPOP_KEY_PORT,
	NM_IPOP_KEY_COMP_LZO,
	NM_IPOP_KEY_MSSFIX,
	NM_IPOP_KEY_TUNNEL_MTU,
	NM_IPOP_KEY_FRAGMENT_SIZE,
	NM_IPOP_KEY_TAP_DEV,
	NM_IPOP_KEY_PROTO_TCP,
	NM_IPOP_KEY_PROXY_TYPE,
	NM_IPOP_KEY_PROXY_SERVER,
	NM_IPOP_KEY_PROXY_PORT,
	NM_IPOP_KEY_PROXY_RETRY,
	NM_IPOP_KEY_HTTP_PROXY_USERNAME,
	NM_IPOP_KEY_CIPHER,
	NM_IPOP_KEY_AUTH,
	NM_IPOP_KEY_TA_DIR,
	NM_IPOP_KEY_TA,
	NM_IPOP_KEY_RENEG_SECONDS,
	NM_IPOP_KEY_TLS_REMOTE,
	NM_IPOP_KEY_REMOTE_RANDOM,
	NULL
};
...
advanced_dialog_new (GHashTable *hash, const char *contype)
{
	...
	value = g_hash_table_lookup (hash, NM_IPOP_KEY_PROXY_SERVER);
	value2 = g_hash_table_lookup (hash, NM_IPOP_KEY_PROXY_PORT);
	if (value && strlen (value) && value2 && strlen (value2)) {
		...
		value = g_hash_table_lookup (hash, NM_IPOP_KEY_PROXY_RETRY);
		if (value && !strcmp (value, "yes"))
			gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (widget), TRUE);

		value = g_hash_table_lookup (hash, NM_IPOP_KEY_HTTP_PROXY_USERNAME);
		if (value && strlen (value)) {
			widget = GTK_WIDGET (gtk_builder_get_object (builder, "proxy_username_entry"));
			gtk_entry_set_text (GTK_ENTRY (widget), value);
		}

		value = g_hash_table_lookup (hash, NM_IPOP_KEY_HTTP_PROXY_PASSWORD);
		if (value && strlen (value)) {
			widget = GTK_WIDGET (gtk_builder_get_object (builder, "proxy_password_entry"));
			gtk_entry_set_text (GTK_ENTRY (widget), value);
		}

		value = g_hash_table_lookup (hash, NM_IPOP_KEY_HTTP_PROXY_PASSWORD"-flags");
		if (value && strlen (value)) {
			errno = 0;
			tmp = strtol (value, NULL, 10);
			if (errno != 0 || tmp < 0 || tmp > 65535)
	...
	value = g_hash_table_lookup (hash, NM_IPOP_KEY_PROXY_TYPE);
	if (value) {
	...
	value = g_hash_table_lookup (hash, NM_IPOP_KEY_TUNNEL_MTU);
	if (value && strlen (value)) {
		long int tmp;
	...
	value = g_hash_table_lookup (hash, NM_IPOP_KEY_FRAGMENT_SIZE);
	if (value && strlen (value)) {
		long int tmp;
	...
	value = g_hash_table_lookup (hash, NM_IPOP_KEY_MSSFIX);
	if (value && !strcmp (value, "yes")) {
		widget = GTK_WIDGET (gtk_builder_get_object (builder, "mssfix_checkbutton"));
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (widget), TRUE);
	}

	value = g_hash_table_lookup (hash, NM_IPOP_KEY_COMP_LZO);
	if (value && !strcmp (value, "yes")) {
	...
	value = g_hash_table_lookup (hash, NM_IPOP_KEY_PROTO_TCP);
	if (value && !strcmp (value, "yes")) {
		widget = GTK_WIDGET (gtk_builder_get_object (builder, "tcp_checkbutton"));
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (widget), TRUE);
	}

	value = g_hash_table_lookup (hash, NM_IPOP_KEY_TAP_DEV);
	if (value && !strcmp (value, "yes")) {
		widget = GTK_WIDGET (gtk_builder_get_object (builder, "tap_checkbutton"));
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (widget), TRUE);
	}

	value = g_hash_table_lookup (hash, NM_IPOP_KEY_REMOTE_RANDOM);
	if (value && !strcmp (value, "yes")) {
	...
	if (   !strcmp (contype, NM_IPOP_CONTYPE_TLS)
		...
		active = -1;
		widget = GTK_WIDGET (gtk_builder_get_object (builder, "tls_auth_checkbutton"));
		value = g_hash_table_lookup (hash, NM_IPOP_KEY_TA);
		if (value && strlen (value))
			gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (widget), TRUE);
		...
		widget = GTK_WIDGET (gtk_builder_get_object (builder, "direction_combo"));
		value = g_hash_table_lookup (hash, NM_IPOP_KEY_TA_DIR);
		if (value && strlen (value)) {
...
advanced_dialog_new_hash_from_dialog (GtkWidget *dialog, GError **error)
{
	...
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) {
		...
		g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_TUNNEL_MTU), g_strdup_printf ("%d", tunmtu_size));
	}
	...
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) {
		...
		g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_FRAGMENT_SIZE), g_strdup_printf ("%d", fragment_size));
	}
	...
	if (proxy_type != PROXY_TYPE_NONE) {
		...
		if (value && strlen (value)) {
			...
			if (proxy_type == PROXY_TYPE_HTTP)
				g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_PROXY_TYPE), g_strdup ("http"));
			else if (proxy_type == PROXY_TYPE_SOCKS)
				g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_PROXY_TYPE), g_strdup ("socks"));

			g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_PROXY_SERVER), g_strdup (value));

			widget = GTK_WIDGET (gtk_builder_get_object (builder, "proxy_port_spinbutton"));
			proxy_port = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (widget));
			if (proxy_port > 0) {
				g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_PROXY_PORT),
				                     g_strdup_printf ("%d", proxy_port));
			}

			widget = GTK_WIDGET (gtk_builder_get_object (builder, "proxy_retry_checkbutton"));
			if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
				g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_PROXY_RETRY), g_strdup ("yes"));
			...
			if (proxy_type == PROXY_TYPE_HTTP) {
				...
				if (value && strlen (value)) {
					g_hash_table_insert (hash,
					                     g_strdup (NM_IPOP_KEY_HTTP_PROXY_USERNAME),
					                     g_strdup (value));
				}
				...
				if (pw_flags != NM_SETTING_SECRET_FLAG_NONE) {
					g_hash_table_insert (hash,
					                     g_strdup (NM_IPOP_KEY_HTTP_PROXY_PASSWORD"-flags"),
	...
	widget = GTK_WIDGET (gtk_builder_get_object (builder, "mssfix_checkbutton"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
		g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_MSSFIX), g_strdup ("yes"));

	widget = GTK_WIDGET (gtk_builder_get_object (builder, "tcp_checkbutton"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
		g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_PROTO_TCP), g_strdup ("yes"));

	widget = GTK_WIDGET (gtk_builder_get_object (builder, "tap_checkbutton"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
		g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_TAP_DEV), g_strdup ("yes"));

	widget = GTK_WIDGET (gtk_builder_get_object (builder, "remote_random_checkbutton"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
		g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_REMOTE_RANDOM), g_strdup ("yes"));
	...
	if (gtk_combo_box_get_active_iter (GTK_COMBO_BOX (widget), &iter)) {
		...
		if (!is_default && cipher) {
			g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_CIPHER), g_strdup (cipher));
		}
	}
	...
	if (gtk_combo_box_get_active_iter (GTK_COMBO_BOX (widget), &iter)) {
		...
		if (!is_default && hmacauth) {
			g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_AUTH), g_strdup (hmacauth));
		}
	}
	...
	if (   !strcmp (contype, NM_IPOP_CONTYPE_TLS)
		...
		if (value && strlen (value))
			g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_TLS_REMOTE), g_strdup (value));
		...
		if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) {
			...
			filename = gtk_file_chooser_get_filename (GTK_FILE_CHOOSER (widget));
			if (filename && strlen (filename)) {
				g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_TA), g_strdup (filename));
			}
			g_free (filename);
			...
			if (gtk_combo_box_get_active_iter (GTK_COMBO_BOX (widget), &iter)) {
				...
				if (direction >= 0) {
					g_hash_table_insert (hash, g_strdup (NM_IPOP_KEY_TA_DIR),
