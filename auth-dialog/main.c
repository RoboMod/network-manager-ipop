             ...
             char **out_certpass)
{
	...
	if (need_password) {
		if (!(pw_flags & NM_SETTING_SECRET_FLAG_NOT_SAVED)) {
			if (in_pass)
				...
				password = keyring_lookup_secret (vpn_uuid, NM_IPOP_KEY_PASSWORD);
		}
	...
	if (need_certpass) {
		if (!(cp_flags & NM_SETTING_SECRET_FLAG_NOT_SAVED)) {
			if (in_certpass)
				...
				certpass = keyring_lookup_secret (vpn_uuid, NM_IPOP_KEY_CERTPASS);
		}
	...
	if (external_ui_mode) {
		...
		if (need_password)
			keyfile_add_entry_info (keyfile, NM_IPOP_KEY_PASSWORD, password ? password : "", _("Password:"), TRUE, allow_interaction);
		if (need_certpass)
			keyfile_add_entry_info (keyfile, NM_IPOP_KEY_CERTPASS, certpass ? certpass : "", _("Certificate password:"), TRUE, allow_interaction);
                    ...
                    gboolean *out_need_certpass)
{
	...
	ctype = g_hash_table_lookup (data, NM_IPOP_KEY_CONNECTION_TYPE);
	g_return_if_fail (ctype != NULL);

	if (!strcmp (ctype, NM_IPOP_CONTYPE_TLS) || !strcmp (ctype, NM_IPOP_CONTYPE_PASSWORD_TLS)) {
		/* Normal user password */
		nm_vpn_plugin_utils_get_secret_flags (data, NM_IPOP_KEY_PASSWORD, &flags);
		if (   !strcmp (ctype, NM_IPOP_CONTYPE_PASSWORD_TLS)
		    && !(flags & NM_SETTING_SECRET_FLAG_NOT_REQUIRED))
			*out_need_password = TRUE;

		/* Encrypted private key password */
		val = g_hash_table_lookup (data, NM_IPOP_KEY_KEY);
		if (val)
...
main (int argc, char *argv[])
{
	...
	if (!need_password && !need_certpass) {
		if (external_ui_mode) {
			...
			g_key_file_set_integer (keyfile, UI_KEYFILE_GROUP, "Version", 2);
			keyfile_add_entry_info (keyfile, NM_IPOP_KEY_NOSECRET, "true", "", TRUE, FALSE);
			keyfile_print_stdout (keyfile);

			g_key_file_unref (keyfile);
		} else {
			/* The older protocol */
			printf ("%s\n%s\n\n\n", NM_IPOP_KEY_NOSECRET, "true");
		}

		return 0;
	}

	nm_vpn_plugin_utils_get_secret_flags (data, NM_IPOP_KEY_PASSWORD, &pw_flags);
	nm_vpn_plugin_utils_get_secret_flags (data, NM_IPOP_KEY_CERTPASS, &cp_flags);
	if (!get_secrets (vpn_name,
	                  ...
	                  allow_interaction,
	                  external_ui_mode,
	                  g_hash_table_lookup (secrets, NM_IPOP_KEY_PASSWORD),
	                  pw_flags,
	                  &new_password,
	                  g_hash_table_lookup (secrets, NM_IPOP_KEY_CERTPASS),
	                  cp_flags,
	                  &new_certpass))
		return 1;  /* canceled */
	...
	if (!external_ui_mode) {
		if (need_password && new_password)
			printf ("%s\n%s\n", NM_IPOP_KEY_PASSWORD, new_password);
		if (need_certpass && new_certpass)
			printf ("%s\n%s\n", NM_IPOP_KEY_CERTPASS, new_certpass);
		printf ("\n\n");
