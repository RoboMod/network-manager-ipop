/* main.c -- dialog to ask use for password
 *
 * Copyright (C) 2004 - 2008 Red Hat, Inc., Dan Williams <dcbw@redhat.com>
...
static gboolean get_secrets(const char *vpn_name,
    ...
    if (external_ui_mode) {
        ...
        g_key_file_set_string(keyfile, UI_KEYFILE_GROUP, "Title",
                              _("Authenticate VPN"));

        if (need_password)
            keyfile_add_entry_info(keyfile, NM_IPOP_KEY_XMPP_PASSWORD,
                                   password ? password : "", _("Password:"),
                                   TRUE, allow_interaction);
         ...
         * without asking the user, or we've got all the passwords we need
         * already.
         */
        if (need_password)
    ...
    dialog = VPN_PASSWORD_DIALOG(vpn_password_dialog_new(_("Authenticate VPN"),
                                                         prompt, NULL));
...
static void get_password_types(GHashTable *data,
                               gboolean *out_need_password) {
    NMSettingSecretFlags flags = NM_SETTING_SECRET_FLAG_NONE;

    /* Normal user password */
    nm_vpn_plugin_utils_get_secret_flags(data, NM_IPOP_KEY_XMPP_PASSWORD,
                                         &flags);
...
int main (int argc, char *argv[]) {
    ...
    GOptionEntry entries[] = {
            ...
            { "external-ui-mode", 0, 0, G_OPTION_ARG_NONE, &external_ui_mode,
              "External UI mode", NULL},
            { NULL }
        };
