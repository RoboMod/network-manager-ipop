...
/* nm-ipop-service-ipop-helper - helper called after IPOP established
 * a connection, uses DBUS to send information back to nm-ipop-service
 *
 ...
 *
 * $Id: nm-ipop-service-ipop-helper.c 4170 2008-10-11 14:44:45Z dcbw $
 * 
 */
...
helper_failed (DBusGConnection *connection, const char *reason)
{
	...
	g_warning ("nm-ipop-service-ipop-helper did not receive a valid %s from ipop", reason);
...
main (int argc, char *argv[])
{
	...
	if (argc >= 2 && !g_strcmp0 (argv[1], "--helper-debug")) {
		g_message ("ipop script environment ---------------------------");
		iter = environ;
		while (iter && *iter)
			g_print ("%s\n", *iter++);
		g_message ("------------------------------------------------------");
